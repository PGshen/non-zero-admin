      ...
      <el-table-column min-width="300px" label="Case_Synopsis">
        <template slot-scope="scope">
          ...
          <router-link :to="'/official-site/customer-case/edit/'+scope.row.id" class="link-type">
            <span>{{ scope.row.customerCaseSynopsis }}</span>
          </router-link>
        </template>
      </el-table-column>
...
import { fetchList, deleteCustomerCase, checkoutStatus } from '@/api/official-site/customer-case/customer-case'
  ...
  methods: {
    ...
    handleDelete(row) {
      this.$confirm('此操作将永久删除, 是否继续?', '提示', {
        ...
        deleteCustomerCase(row.id).then(response => {
